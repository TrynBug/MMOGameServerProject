...
namespace MMO.Client.Navigation
{
    ...
    /// - Recast/Detour는 입력 삼각형을 받아 walkable 영역을 계산하는데,
    ///   시각용 메시까지 입력에 넣으면 (1) 빌드가 느려지고 (2) 결과가 지저분해진다.
    /// - 그래서 "이 게임오브젝트는 NavMesh 빌드에 포함시켜라"는 명시적 표시가 필요하다.
    /// - 이 컴포넌트가 그 마커 역할을 한다.
    ///
    /// 수집 대상은 **Collider 로 일원화**되어 있다:
    /// - 하위의 Collider(Box/Sphere/Capsule/Mesh)와 TerrainCollider 를 nav 입력으로 수집.
    /// - 비활성(GameObject/Collider) 및 isTrigger 콜라이더는 제외.
    /// - layerMask 로 특정 레이어만 수집하도록 좁힐 수 있음(기본 Everything).
    /// - 콜라이더 없는 시각 전용 오브젝트(풀/꽃 등)는 자동으로 nav 에서 제외된다.
    ///
    /// 사용법:
    /// - 충돌/내비 지오메트리(지면 콜라이더/터레인 + 장애물 콜라이더)의 루트에 이 컴포넌트를 붙인다.
    /// - includeChildren=true (기본) 면 자식 트리의 모든 Collider 가 자동 수집된다.
    ///
    ...
    public class NavMeshSource : MonoBehaviour
    {
        ...
        /// </summary>
        [Tooltip("자식 오브젝트의 Collider 도 함께 수집합니다.")]
        public bool includeChildren = true;

        // =====================================================================
        // 레이어 필터 (선택)
        //
        // nav 입력으로 쓸 Collider 의 레이어를 제한한다. 기본은 모든 레이어(Everything).
        // 물리 충돌과 nav 를 분리하고 싶을 때(예: "NavBlocker" 레이어만 nav 로) 좁힌다.
        // 기본값이 Everything 이라 평소엔 신경 쓸 필요 없다.
        // =====================================================================

        [Tooltip("이 레이어들의 Collider 만 nav 입력으로 수집한다. 기본 Everything.")]
