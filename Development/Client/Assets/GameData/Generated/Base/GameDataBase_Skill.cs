...
namespace GameData
{
    ...
    public class GameDataTableBase_Skill : GameDataTableBase
    {
        ...
        protected override bool MakeGameData(string line)
        {
            ...
            data.SfxCast = fields[7];
            data.SfxShoot = fields[8];
            data.SfxLoop = fields[9];
            data.SfxHit = fields[10];
            data.SfxHitIgnoreMs = int.Parse(fields[11]);
            data.Job = (EJob)int.Parse(fields[12]);
            data.CastClass = (ESkillCastClass)int.Parse(fields[13]);
            data.IsPrimaryEligible = GameDataTableBase.StringToBool(fields[14]);
            data.OnHitSkillKey = int.Parse(fields[15]);
            data.NextSkillKey = int.Parse(fields[16]);
            data.NextTriggerTiming = (ENextSkillTiming)int.Parse(fields[17]);
            data.NextTriggerDelayMs = int.Parse(fields[18]);
            data.NextOrigin = (ENextSkillOrigin)int.Parse(fields[19]);
            data.CasterFrontDistance = float.Parse(fields[20]);
            data.CooldownMs = int.Parse(fields[21]);
            data.ManaCost = float.Parse(fields[22]);
            data.CastDelayMs = int.Parse(fields[23]);
            data.ActionLockMs = int.Parse(fields[24]);
            data.Rotation = GameDataTableBase.StringToBool(fields[25]);
            data.Targeting = (ETargetingMode)int.Parse(fields[26]);
            data.Placement = (ESkillPlacement)int.Parse(fields[27]);
            data.EffectMotion = (ESkillEffectMotion)int.Parse(fields[28]);
            data.EffectDamage = (ESkillEffectDamage)int.Parse(fields[29]);
            data.EffectShape = (ESkillEffectShape)int.Parse(fields[30]);
            data.DamageCoeff = double.Parse(fields[31]);
            data.Radius = float.Parse(fields[32]);
            data.ObbWidth = float.Parse(fields[33]);
            data.ObbLength = float.Parse(fields[34]);
            data.ProjectileSpeed = float.Parse(fields[35]);
            data.MaxRange = float.Parse(fields[36]);
            data.ProjectileCount = int.Parse(fields[37]);
            data.FanAngleDeg = float.Parse(fields[38]);
            data.FirstTickDelayMs = int.Parse(fields[39]);
            data.TickIntervalMs = int.Parse(fields[40]);
            data.LifetimeMs = int.Parse(fields[41]);
            data.MoveDistance = float.Parse(fields[42]);
            data.MoveDurationMs = int.Parse(fields[43]);
            data.ScatterCount = int.Parse(fields[44]);
            data.ScatterInnerRadius = float.Parse(fields[45]);
            data.ScatterOuterRadius = float.Parse(fields[46]);
