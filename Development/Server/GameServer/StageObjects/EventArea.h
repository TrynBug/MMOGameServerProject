...
public:
    ...
    // occupant(현재 영역 안에 있는 objectId) 관리 — 진입/이탈 중복방지·이탈판정용.
    bool AddOccupant(int64 objectId)    { return m_occupants.insert(objectId).second; }   // 신규 진입이면 true
    bool RemoveOccupant(int64 objectId) { return m_occupants.erase(objectId) != 0; }       // 안에 있었으면 true
